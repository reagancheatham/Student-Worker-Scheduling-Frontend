...
export class EmployeeUnavailabilityEventData extends EventData {
    constructor(
        public readonly employeeUnavailability: EmployeeUnavailability,
    ) {
        super(
            employeeUnavailability.name,
            EventTime.fromDate(employeeUnavailability.startTime),
            EventTime.fromDate(employeeUnavailability.endTime),
            EventColor.mist,
        );
    }
    ...
    public override isValid(): boolean {
        return this.employeeUnavailability.isValid();
    }

    getClass(styleData: EventStyleData): string {
        return `event ring-inset ring-3 ${this.color.ring}`;
    }

    getStyle(styleData: EventStyleData) {
    ...
    getLabel(styleData: EventStyleData): string {
        return this.name;
    }

    getGridArea(calendarData: CalendarData): string {
        ...
        if (calendarData.selectedView === CalendarMode.Day) {
            ...
            const employee = this.employeeUnavailability.employee;
